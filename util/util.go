...
// IsValidVPCRegion validates whether PowerVS zone passed is valid or not.
func IsValidPowerVSZone(zone string) bool {
	if r, ok := Regions[endpoints.ConstructRegionFromZone(zone)]; ok {
...
// IsValidVPCRegion validates whether VPC region passed is valid or not.
func IsValidVPCRegion(region string) bool {
	for _, pvsRegion := range Regions {
		if pvsRegion.VPCRegion == region {
			return true
		}
	}
	return false
